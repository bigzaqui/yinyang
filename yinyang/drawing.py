...
def get_descriptor(path,desc_set):

    for hop in path:
        desc_set.add(hop['descriptor'])
    return desc_set
...
def get_descriptor_map(trace, node_desc):
    for hop in trace:
            node_desc[hop['descriptor']] = hop['nodetype']
    return node_desc

def get_nodelist(node_set,node_desc, node_index, nodetype):
    nodelist = []
    for node in node_set:
        if node_desc[node] == nodetype:
